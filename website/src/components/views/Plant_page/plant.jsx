...
function PlantPage() {
  ...
  const fetchPlants = () => {
    axios.get("http://localhost:5001/plants")
      .then((response) => setPlants(response.data))
      .catch((error) => console.error("Lỗi khi lấy dữ liệu cây trồng:", error));
  };

  const handleAddPlant = () => {
    axios.post("http://localhost:5001/plants", newPlant)
      .then(() => {
        fetchPlants();
  ...
  const handleDeletePlant = (id) => {
    axios.delete(`http://localhost:5001/plants/${id}`)
      .then(() => {
        fetchPlants();
      })
        ...
        <div className="fixed inset-0 flex justify-end bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-[600px]">
            <div className="flex justify-end space-x-2 mt-4">
              <button className="bg-mint px-4 py-2 rounded-lg " onClick={handleCloseDetailModal}>Đóng</button>
            </div>
            <h2 className="text-lg font-bold mb-4 mt-5">Chi tiết cây trồng</h2>
            <p><strong>Tên cây:</strong> {currentPlant.plant_name}</p>
            
            <div className="mt-6">
              <h3 className="text-lg font-semibold mb-4">Tiến độ tăng trưởng</h3>
              <div className="flex items-center justify-center">
                
              </div>
              <div className="mt-4 text-center text-sm text-gray-600">
                <p>Bắt đầu: {currentPlant.start_date}</p>
                <p>Kết thúc: {currentPlant.end_date}</p>
              </div>
            </div>
          </div>
        </div>
